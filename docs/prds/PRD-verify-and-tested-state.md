...
**DECISION (locked in ADR-012 D2): Direction B — `tested` does NOT satisfy hard dependencies.**

The dep gate's satisfaction set stays `{applied, upstream_merged}`. `tested` is a stronger label than `applied` for every other purpose (status display, harness handoff), but for the dep gate it is treated as "applied" — i.e., `tested` parents satisfy because they are `applied` underneath, not because they are `tested`.

This is implemented trivially: the `tested` → `applied` demotion is the only state-flip semantically; the gate need not learn about `tested` at all because by the time the gate runs, the parent's persisted state for gate purposes is the underlying `applied` claim plus the `tested` overlay.

**Wait — clarify.** In the §3.4.1 enum, `tested` is a value of `FeatureState`. So a parent in state `tested` is *not* in state `applied`; the gate would reject it under Direction B. Two ways to resolve:

- **B-strict**: `CheckDependencyGate` switch gains an explicit `case StateTested:` that **rejects** with state="tested" in the blocker message. Operator sees `parent foo is tested but apply gate requires applied/upstream_merged; run tpatch apply <foo>`. This is correct but UX-hostile.
- **B-pragmatic** (locked): The gate's satisfaction set becomes `{applied, tested, upstream_merged}` because `tested` is by construction a strict superset of `applied`'s structural guarantees. The locked decision is therefore **"`tested` satisfies, but only because it implies `applied`-level guarantees; it gains no extra power vs. `applied` for gating purposes."** The implementation: extend the `case StateApplied:` arm in `CheckDependencyGate` to also match `StateTested`. The change is exactly two lines and has no other behavioural effect.

The disorienting demotion case described under "Pros of B" is mitigated by the §3.4.5 parent-state hook: when a parent demotes from `tested` → `applied`, **children are not affected** (no cascade), but `tpatch status --dag` surfaces a `stale-parent-applied` label per existing v0.6.1 logic (no new label needed — the existing `LabelStaleParentApplied` is exactly this signal).

To be clear: ADR-012 D2 locks **B-pragmatic** = "`tested` satisfies the hard-dep gate; does not change soft-dep semantics; does not change `created_by` semantics; demotion does not cascade to children."

**Deliberately rejected alternative (recorded in ADR-012 D2 alt 3):** "`tested` produces a stricter form of dep satisfaction — children of a `tested` parent inherit a `parent-tested` label." Rejected because it bloats the M14.3 label set (`waiting-on-parent`, `blocked-by-parent`, `stale-parent-applied`) for no operator-actionable benefit, and re-opens ADR-011 D3 (composable derived labels) to extend the label vocabulary for `tested`. Out of scope; revisit in a hypothetical `feat-tested-cascade-labels` if a real harness need surfaces.
